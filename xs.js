...
function tvboxFromHex(hex) {
  if (!/^[0-9a-f]+$/i.test(hex) || hex.length % 2) throw new Error("Invalid TVBox hex data");
...
async function tvboxKey(value) {
  return crypto.subtle.importKey("raw", tvboxWord(value), { name: "AES-CBC" }, false, ["encrypt", "decrypt"]);
}

function tvboxJsonValue(text) {
  try { return JSON.parse(text.trim()); } catch (_) { return text; }
}
...
async function tvboxEncrypt(text, key, iv) {
  ...
  return tvboxHex(tvboxBytes(`$#${key}#$`)) +
    tvboxHex(new Uint8Array(encrypted)) + tvboxHex(tvboxBytes(iv));
}

async function tvboxDecrypt(text) {
  // Correct JavaScript regex: remove lines beginning with //.
  const input = text.replace(/^\/\/.*$/gm, "").trim();
  if (!input) return input;

  try { return JSON.stringify(JSON.parse(input), null, 2); } catch (_) {}
...
async function processLines(text, p) {
  ...
  if (p.tvbox === 1) return [await tvboxEncrypt(lines.join("\n"), p.tvboxKey || p.key, p.tvboxIv || p.key)];
  return lines;
}
...
async function mergeResponse(request) {
  ...
  let finalLines = [];
  for (const parts of merge.split(",").map(item => item.split("|")).filter(parts => parts[0])) {
  ...
  if (p.tvbox === 0) {
    try {
      finalLines = (await tvboxDecrypt(finalLines.join("\n"))).split(/\r?\n/).filter(x => x.trim());
    } catch (_) {
      const decrypted = [];
  ...
  let output = finalLines.filter(x => x.trim()).join("\n");
