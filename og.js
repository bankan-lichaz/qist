export default {
  async fetch(request) {
    ...
    const g_tvbox = searchParams.get("tvbox");
    const g_tvboxKey =
      searchParams.get("tvboxKey") ||
      searchParams.get("tvkey") ||
      g_key;

    const g_tvboxIv =
      searchParams.get("tvboxIv") ||
      searchParams.get("tviv") ||
      g_key;
...
function normalizeChannelName(line) {
  ...
  return `${clean},${url}`;
    ...
    async function tvboxEncrypt(text, key, iv) {
      if (!key || !iv) {
        throw new Error("TVBox encryption requires key and iv");
      }
      ...
      const encrypted = await crypto.subtle.encrypt(
        ...
        await tvboxKey(key),
        tvboxBytes(JSON.stringify(tvboxJsonValue(text), null, 2))
      );

      return (
        tvboxHex(tvboxBytes(`$#${key}#$`)) +
        tvboxHex(new Uint8Array(encrypted)) +
        tvboxHex(tvboxBytes(iv))
      );
    }
    ...
    async function tvboxDecrypt(text) {
      ...
      if (marker < 0) {
        throw new Error("TVBox header was not found");
      }
      ...
      const realKey = new TextDecoder()
        .decode(tvboxFromHex(input.slice(0, headerEnd)))
        .slice(2, -2);

      const realIv = new TextDecoder()
        .decode(tvboxFromHex(ivHex));
    ...
    // 单源独立处理
    async function processSingleSource(text, params) {
      if (params.tvbox === 0) {
        text = await tvboxDecrypt(text);
      }
...
if (params.cut) {
  ...
  lines = result.length > 0 ? result : [];
    ...
    if (params.tvbox === 1) {
      const key = params.tvboxKey || params.key;
      const iv = params.tvboxIv || params.key;

      return [
        await tvboxEncrypt(out.join("\n"), key, iv)
      ];
    }
    ...
    const sources = mergeRaw.split(",").map(item => {
      ...
      for (let i = 1; i < parts.length; i++) {
        ...
        const k = pos < 0 ? parts[i] : parts[i].slice(0, pos);
        const v = pos < 0 ? "" : parts[i].slice(pos + 1);
...
for (let src of sources) {
  ...
  if (src.params.multiurl) {
    ...
    //异步函数multiurl调用
    const processed = await processSingleSource(
      generatedText,
      newParams
    );
  ...
  //异步函数普通调用
  let processed = await processSingleSource(raw, src.params);
  finalLines.push(...processed);
}
...
if (g_tvbox === "0") {
  try {
    finalLines = (
      await tvboxDecrypt(finalLines.join("\n"))
    )
      .split(/\r?\n/)
    ...
    for (const line of finalLines) {
      try {
        decrypted.push(await tvboxDecrypt(line));
      } catch (_) {
        decrypted.push(line);
      }
    }
